...
def run(mock_semhash):
    ...
    print("Mocks prepared")
    print(mock_semhash.from_records.return_value.self_deduplicate.return_value.selected)
    
    result = processor._process_with_semhash(df)
    
    print("Call count from_records:", mock_semhash.from_records.call_count)
